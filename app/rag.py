...
class RagService:
    ...
    def ingest_book(self, book: BookFile, index: int | None = None, total: int | None = None) -> dict:
        ...
        try:
            inserted_count = self._embed_and_upsert_chunks(book, chunks, log_prefix)
        except Exception as exc:
            result["status"] = "failed"
            result["error"] = str(exc)
    ...
    def _embed_and_upsert_chunks(self, book: BookFile, chunks: list[dict], log_prefix: str) -> int:
        point_ids = []
        point_buffer = []
        inserted_count = 0
        ...
        for batch_index, start in enumerate(range(0, len(chunks), EMBEDDING_BATCH_SIZE), start=1):
            ...
            while len(point_buffer) >= QDRANT_UPSERT_BATCH_SIZE:
                ...
                inserted_count += self._flush_point_batch(
        ...
        self._mark_points_complete(point_ids, log_prefix)
    ...
    def _mark_points_complete(self, point_ids: list[str], log_prefix: str) -> None:
